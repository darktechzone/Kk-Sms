# ╔══════════════════════════════════════════════════════════════╗
# ║     ULTIMATE PANEL SCRAPER – FULL MAPS + DEBUG            ║
...
# ---------- SESSION MANAGER (with better error handling) ----------
class PanelSession:
    def __init__(self):
        self.session = None
        self.sesskey = None
...
def root():
    return jsonify({
        "message": "Panel Scraper – Full Maps + Debug",
        "endpoints": ["/numbers", "/sms", "/logs", "/debug", "/login-status"],
        "status": "online"
    })
...
def numbers():
    try:
        data = fetch_numbers()
        if data is None:
            return jsonify({"success": False, "error": "Failed to fetch numbers – check /logs"}), 500
...
def login_status():
    """Check if session is valid and login works."""
    if not panel.session:
        ok = panel.login()
        return jsonify({"logged_in": ok, "session_exists": False if not panel.session else True})
    valid = panel._validate()
    return jsonify({
        "logged_in": valid,
        "session_exists": panel.session is not None,
        "sesskey": panel.sesskey
    })

@app.route("/debug")
def debug():
    """Show raw response from the numbers endpoint for debugging."""
    url = f"{BASE_URL}/agent/res/data_smsnumbers.php"
    params = {
        "frange": "", "fclient": "", "fnumber": "",
        "sEcho": "1", "iDisplayStart": "0", "iDisplayLength": "1",
