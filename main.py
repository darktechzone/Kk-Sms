# ╔══════════════════════════════════════════════════════════════╗
# ║     UNKILLABLE PANEL SCRAPER – WORM EDITION                ║
# ║     Mimics Node.js resilience – survives password changes   ║
...
class PanelSession:
    def __init__(self):
        ...
        self.BREAKER_TIMEOUT = 30  # seconds
    ...
    def _validate(self):
        """Check if current session is still valid by hitting a lightweight endpoint."""
        try:
    ...
    def _keep_alive_loop(self):
        """Send a ping every 5 minutes to keep session alive."""
        while self._keep_alive_running:
            time.sleep(300)  # 5 minutes
            try:
                if not self.session:
                    continue
                # Hit any endpoint to refresh session
                self.session.get(
                    BASE_URL + "/agent/MySMSNumbers2",
                    headers=HEADERS,
                    timeout=5
                )
    ...
    def ensure_session(self):
        ...
        if self.consecutive_failures >= self.FAILURE_THRESHOLD:
            print(f"[CIRCUIT] Too many failures, cooling down {self.BREAKER_TIMEOUT}s")
    ...
    def request(self, method, url, **kwargs):
        """Make a request with automatic retry and session re‑login on failure."""
        ...
        try:
            ...
            if resp.status_code in (503, 403):
                # Session might be dead – re‑login and retry once
...
def fetch_otps_raw(limit=10):
    """Fetch OTPs from panel using the session manager."""
...
# ---------- BACKGROUND KEEP-ALIVE AND CACHE REFRESH ----------
def background_tasks():
    while True:
        time.sleep(30)  # refresh cache every 30 seconds
        refresh_cache()
...
def root():
    return jsonify({
        "message": "Unkillable Panel Scraper – Worm Edition",
...
def numbers():
    try:
        # Fetch numbers using session manager
        url = f"{BASE_URL}/agent/res/data_smsnumbers.php"
        params = {
...
if __name__ == "__main__":
    ...
    if panel.login():
        print("[INIT] ✅ Initial login successful.")
        # Start background tasks
        threading.Thread(target=background_tasks, daemon=True).start()
    else:
        print("[INIT] ❌ Initial login failed. Endpoints may return errors.")
