# ╔══════════════════════════════════════════════════════════════╗
# ║     PANEL SCRAPER – KONEKTA PREMIUM (konektapremium.net)  ║
# ║     Endpoints: /numbers, /sms, /logs                      ║
# ║     Auto‑relogin + keep‑alive – survives password changes ║
...
# ---------- CONFIG (NEW PANEL) ----------
BASE_URL = os.environ.get("PANEL_BASE_URL", "https://konektapremium.net")
...
class PanelSession:
    ...
    def _get_captcha_answer(self, html):
        # Try to find captcha question – adapt if needed
        m = re.search(r'What is (\d+) \+ (\d+) = \?', html)
        if m:
            return int(m[1]) + int(m[2])
        # If not found, maybe it's a different format – try fallback
    ...
    def login(self):
        ...
        # Try both login paths; the new panel might have /sign-in only
        login_paths = ["/sign-in", "/login"]
        success = False
        for path in login_paths:
            try:
                ...
                # The sign-in endpoint might be /sign-in – but we already used that path for GET
                # The POST action might be the same URL or a different one like /sign-in
                signin_url = f"{BASE_URL}/sign-in"  # assuming same URL for POST
        ...
        r3 = self.session.get(
            ...
            headers={"Referer": f"{BASE_URL}/agent/SMSCDRStats"},
            timeout=10
        )
    ...
    def _keep_alive_loop(self):
        while self._keep_alive_running:
            ...
            try:
                if self.session:
                    self.session.get(f"{BASE_URL}/agent/MySMSNumbers2", headers=HEADERS, timeout=5)
...
def root():
    return jsonify({
        "message": "Panel Scraper – Konekta Premium",
        "endpoints": ["/numbers", "/sms", "/logs"],
        "status": "online"
    })
...
if __name__ == "__main__":
    if panel.login():
        ...
        logger.error("❌ Initial login failed. Endpoints may error.")
