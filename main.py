# ╔══════════════════════════════════════════════════════════════╗
# ║     VERCEL-READY PANEL SCRAPER – PYTHON (Node.js clone)   ║
...
class PanelSession:
    ...
    def _validate(self):
        try:
            ...
            if resp.status_code in (503, 403):
                logger.warning(f"[VALIDATE] {resp.status_code}, waiting 3s...")
                time.sleep(3)
    ...
    def _keep_alive_loop(self):
        """Every 30 seconds, refresh cache to keep session alive."""
        while self._keep_alive_running:
            ...
            try:
                ...
                # Just hit any endpoint to keep session alive
...
def get_cached_otps():
    ...
    if otp_cache["data"] and age < CACHE_FALLBACK:
        if age > CACHE_TTL:
            # Refresh in background (sync for simplicity)
            refresh_cache()
...
def root():
    return jsonify({
        "message": "Panel Scraper – Python (Node.js clone)",
        "endpoints": ["/api/numbers", "/api/otps/last10", "/logs"],
        "status": "online"
    })
...
def api_numbers():
    try:
        ...
        logger.error(f"/api/numbers error: {e}")
        return jsonify({"success": False, "error": str(e)}), 500

@app.route("/api/otps/last10")
def api_otps():
    try:
        ...
        logger.error(f"/api/otps error: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
...
# ---------- STARTUP (for local, Vercel uses serverless) ----------
if __name__ == "__main__":
    # On startup, login and start keep-alive thread
    if panel.login():
        logger.info("✅ Initial login successful.")
        # Do an initial cache refresh
        refresh_cache()
    else:
